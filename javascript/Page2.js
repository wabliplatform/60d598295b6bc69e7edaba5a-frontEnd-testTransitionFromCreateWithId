...
if(insideSubDataElement !== null && data[data.length -i -1].pImage !== undefined){
  ...
  subDataElements[i].name = data[data.length -i -1].pImage.name;
      ...
      const insideSubDataElement = subDataElements[i].querySelector("[annotationname = 'pDescription']");
      if(insideSubDataElement !== null){
        insideSubDataElement.textContent = data[data.length -i -1].pDescription;
        
      }
      else if(subDataElements[i].getAttribute('annotationname') === 'pDescription'){
        subDataElements[i].textContent = data[data.length -i -1].pDescription;
